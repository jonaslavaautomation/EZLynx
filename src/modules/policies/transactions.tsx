...
type Props = { policy: Policy; onClose: () => void };
...
function EndorseModal({ policy, onClose }: Props) {
  const log = useLog(policy);
  const [date, setDate] = useState(today());
  const [desc, setDesc] = useState('');
  const [change, setChange] = useState('0');
  const delta = parseAmount(change);
  const newPremium = cents(Number(policy.premium) + (delta ?? 0));
  return (
    <TxModal title="Endorse / change policy" subtitle={sub(policy)} confirmLabel="Process endorsement" onClose={onClose} run={async () => {
      ...
      need(date >= policy.effective_date && date <= policy.expiration_date, 'Effective date must fall within the current term.');
      ...
      await log(`Endorsement processed — ${policy.policy_number}`, `${desc.trim()} (effective ${fmtDate(date)}, premium ${signedMoney(delta!, fmtMoney)})`);
      return 'Endorsement recorded';
    }}>
      <Summary items={[['Current premium', fmtMoney(policy.premium, true)], ['Change', delta === null ? '—' : signedMoney(delta, fmtMoney)], ['New premium', fmtMoney(newPremium, true)], ['Term ends', fmtDate(policy.expiration_date)]]} />
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <Field label="Effective date" required><Input type="date" value={date} min={policy.effective_date} max={policy.expiration_date} onChange={(e) => setDate(e.target.value)} /></Field>
...
function CancelModal({ policy, onClose }: Props) {
  const log = useLog(policy);
  const initialDate = today() < policy.effective_date ? policy.effective_date : today() > policy.expiration_date ? policy.expiration_date : today();
  const [date, setDate] = useState(initialDate);
  const [reason, setReason] = useState(CANCEL_REASONS[0]);
  const [details, setDetails] = useState('');
  const [ret, setRet] = useState(String(proRataReturn(policy, initialDate)));
  const [autoRet, setAutoRet] = useState(true);
  const r = parseAmount(ret);
  const changeDate = (d: string) => { setDate(d); if (autoRet && d) setRet(String(proRataReturn(policy, d))); };
  return (
    <TxModal title="Cancel policy" subtitle={sub(policy)} confirmLabel="Cancel policy" danger onClose={onClose} run={async () => {
      ...
      need(date >= policy.effective_date && date <= policy.expiration_date, 'Cancellation date must fall within the current term.');
      ...
      need(r! <= Number(policy.premium), 'Return premium cannot exceed the term premium.');
      const desc = `Cancelled effective ${fmtDate(date)} — ${reason}${details.trim() ? `: ${details.trim()}` : ''}. Return premium ${fmtMoney(r!, true)}.`;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <Field label="Cancellation date" required><Input type="date" value={date} min={policy.effective_date} max={policy.expiration_date} onChange={(e) => changeDate(e.target.value)} /></Field>
        ...
        <div className="flex items-end pb-1">
          <Button size="sm" variant="ghost" onClick={() => { setAutoRet(true); setRet(String(proRataReturn(policy, date))); }}>Use pro-rata</Button>
        </div>
...
function ReinstateModal({ policy, onClose, lastCancel }: Props & { lastCancel?: PolicyTransaction }) {
  const log = useLog(policy);
  ...
  return (
    <TxModal title="Reinstate policy" subtitle={sub(policy)} confirmLabel="Reinstate" onClose={onClose} run={async () => {
      ...
      need(date >= policy.effective_date && date <= policy.expiration_date, 'Reinstatement date must fall within the current term.');
      ...
      await db.update('policies', policy.id, { status: 'Active' });
...
export function TransactionModal({ kind, policy, onClose, lastCancel }: Props & { kind: TxKind; lastCancel?: PolicyTransaction }) {
  switch (kind) {
    case 'endorse': return <EndorseModal policy={policy} onClose={onClose} />;
    case 'renew': return <RenewModal policy={policy} onClose={onClose} />;
    case 'cancel': return <CancelModal policy={policy} onClose={onClose} />;
    case 'reinstate': return <ReinstateModal policy={policy} onClose={onClose} lastCancel={lastCancel} />;
